						...
						<view class="comments-wrapper"
							...
							<view class="comments-wrapper-sub-up">
								<view class="vlogger-wrapper">
									...
									<view style="margin-left: 10px;width: 456rpx;">
										<view style="display: flex;flex-direction: row;">
											...
											<image
												v-if="commentContent.replayedUserNickname != null  && commentContent.replayedUserNickname != ''"
											...
											<text
												v-if="commentContent.replayedUserNickname != null  && commentContent.replayedUserNickname != ''"
												style="font-size: 30rpx;color: #878585;align-self: center;">{{commentContent.replayedUserNickname}}</text>
										</view>
										<text
								...
								<view style="display: flex;flex-direction: column;width: 26px;">
									...
									<text
										style="font-size: 22rpx;color: #878585;align-self: center;">{{getGraceNumber(commentContent.likeCounts)}}</text>
								</view>
							</view>
							<view class="comments-wrapper-sub-down">
								<image src="/static/face/face-arrow-1.png" class="img-face" style="opacity: 0;"></image>
								<text
									style="font-size: 30rpx;color: #878585;margin-left: 10px;">{{getGraceDateBeforeNow(commentContent.createdTime)}}</text>
	...
	export default {
		...
		methods: {
					...
					success: function(res) {
						if (res.confirm) {
							...
							uni.request({
								...
								url: serverUrl + "/comment/delete?commentUserId=" + commentUserId +
									"&commentId=" + commentId + "&vlogId=" + vlogId,
								success(result) {
									console.log(result);
			...
			doCommentPagingList(page, needClearList) {
				...
				uni.request({
					method: "GET",
					url: serverUrl + "/comment/list?vlogId=" + vlogId + "&userId=" + me.loginUserId + "&current=" +
						page + "&pageSize=10",
					success(result) {
						// console.log(result);
