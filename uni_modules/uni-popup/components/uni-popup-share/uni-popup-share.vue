...
<template>
	<view class="page">
		...
		<view class="all-box">
			<view class="share-item" @click="downloadVlog()">
				<view class="icon-wrapper" style="align-self: center;">
					<image class="icon-image" src="/static/images/icon-download.png" style="align-self: center;">
					</image>
				</view>
		...
		<view class="" style="padding: 0 20rpx;">
			...
			<view :class="{'btn-cancel':!cancelTouched, 'btn-cancel-touched': cancelTouched}" @click="close"
				@touchstart="touchstartCancel" @touchend="touchendCancel"
		...
		methods: {
			...
			changeVlogToPublic() {
				...
				uni.request({
					...
					url: serverUrl + "/vlog/changePrivacy?userId=" + userId + "&vlogId=" + vlogId +
						"&isPrivate=0",
					success(result) {
						console.log(result);
